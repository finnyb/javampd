...
public class MPDServerStatistics implements ServerStatistics {
  ...
  /**
   * Returns the statistics of the requested statistics element. See <code>StatList</code> for a
   * list of possible items returned by getServerStat.
   *
      ...
      if (line.startsWith(stat.getStatPrefix())) {
        try {
          return (NumberFormat.getInstance())
              .parse(line.substring(stat.getStatPrefix().length()).trim());
