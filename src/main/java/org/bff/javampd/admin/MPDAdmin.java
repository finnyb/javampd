...
public class MPDAdmin implements Admin {
  ...
  public Collection<MPDOutput> getOutputs() {
    return new ArrayList<>(
      parseOutputs(commandExecutor.sendCommand(adminProperties.getOutputs()))
    );
  }
  ...
  public boolean enableOutput(MPDOutput output) {
    fireOutputChangeEvent(OUTPUT_EVENT.OUTPUT_CHANGED);
    return commandExecutor
      .sendCommand(adminProperties.getOutputEnable(), output.getId())
      .isEmpty();
  }

  private static Collection<MPDOutput> parseOutputs(
  ...
  private static MPDOutput parseOutput(
    String startingLine,
    Iterator<String> iterator
  ) {
    MPDOutput output = new MPDOutput(
    ...
    while (!line.startsWith(OUTPUT_PREFIX_ID)) {
      if (line.startsWith(OUTPUT_PREFIX_NAME)) {
        ...
        output.setEnabled(
          "1".equals(line.replace(OUTPUT_PREFIX_ENABLED, "").trim())
        );
      }
      if (!iterator.hasNext()) {
        break;
