...
public class MPDPlaylist implements Playlist {
  ...
  @Override
  public synchronized void addPlaylistChangeListener(
    PlaylistChangeListener pcl
  ) {
    listeners.add(pcl);
  }

  @Override
  public synchronized void removePlaylistStatusChangedListener(
    PlaylistChangeListener pcl
  ) {
   ...
   */
  protected synchronized void firePlaylistChangeEvent(
    PlaylistChangeEvent.Event event
  ) {
  ...
  public boolean addSongs(List<MPDSong> songList, boolean fireEvent) {
    commandExecutor.sendCommands(
      songList
        .stream()
        .map(
          song -> new MPDCommand(playlistProperties.getAdd(), song.getFile())
        )
    ...
    if (fireEvent) {
      firePlaylistChangeEvent(
        PlaylistChangeEvent.Event.MULTIPLE_SONGS_ADDED,
        Integer.toString(songList.size())
      );
    }
  ...
  public void removeSong(MPDSong song) {
    if (song.getId() > -1) {
      ...
      commandExecutor.sendCommand(
        playlistProperties.getRemoveId(),
        song.getId()
      );
    } else {
  ...
  public MPDSong getCurrentSong() {
    List<MPDSong> songs = convertResponseToSong(
      commandExecutor.sendCommand(playlistProperties.getCurrentSong())
    );
  ...
  public void move(MPDSong song, int to) {
    if (song.getId() > -1) {
      commandExecutor.sendCommand(
        playlistProperties.getMoveId(),
        song.getId(),
        to
      );
    } else if (song.getPosition() > -1) {
      commandExecutor.sendCommand(
        playlistProperties.getMove(),
        song.getPosition(),
        to
      );
    }
  ...
  public void swap(MPDSong song1, MPDSong song2) {
    if (song1.getId() > -1 && song2.getId() > -1) {
      commandExecutor.sendCommand(
        playlistProperties.getSwapId(),
        song1.getId(),
        song2.getId()
      );
    } else if (song1.getPosition() > -1 && song2.getPosition() > -1) {
      commandExecutor.sendCommand(
        playlistProperties.getSwap(),
        song1.getPosition(),
        song2.getPosition()
      );
    }
  ...
  private List<MPDSong> listSongs() {
    return convertResponseToSong(
      commandExecutor.sendCommand(playlistProperties.getInfo())
    );
  }
  ...
  public void removeAlbum(String artistName, String albumName) {
    List<MPDSong> removeList = getSongList()
      .stream()
      .filter(
        song ->
