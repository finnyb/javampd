...
public class MPDSocket {
    ...
    if (line != null && isResponseOK(line)) {
      ...
      throw new MPDConnectionException(
        ...
        (
          (line == null)
            ? "null"
            : stripResponse(responseProperties.getError(), line)
        )
      );
    }
  }
  ...
  private void connectSocket(int timeout) {
    LOGGER.debug(
      "attempting to connect socket to {} with timeout of {}",
      server,
      timeout
    );
  ...
  public synchronized Collection<String> sendCommand(MPDCommand command) {
    ...
    while (count < TRIES) {
      try {
        return sendBytes(
          convertCommand(command.getCommand(), command.getParams())
        );
  ...
  private boolean isResponseOK(final String line) {
    try {
      if (
        line.startsWith(responseProperties.getOk()) ||
  ...
  private boolean isResponseError(final String line) {
    if (line.startsWith(responseProperties.getError())) {
      this.lastError =
        line.substring(responseProperties.getError().length()).trim();
      return true;
    } else {
      return false;
    }
  ...
  public synchronized void sendCommands(List<MPDCommand> commandList) {
    StringBuilder sb = new StringBuilder(
      convertCommand(serverProperties.getStartBulk())
    );
  ...
  private void checkConnection() {
    ...
    if (this.closed) {
      throw new MPDConnectionException(
        "Close has been called on MPD.  Create a new MPD."
      );
    }
  ...
  private void writeToStream(String command) throws IOException {
    socket
      .getOutputStream()
      .write(command.getBytes(serverProperties.getEncoding()));
  }
