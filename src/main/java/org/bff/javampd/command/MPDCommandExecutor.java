...
public class MPDCommandExecutor implements CommandExecutor {
  ...
  @Override
  public synchronized List<String> sendCommand(
    String command,
    String... params
  ) {
  ...
  public synchronized List<String> sendCommand(MPDCommand command) {
    try {
      ...
      LOGGER.warn(
        "Connection exception while sending command {}, will retry",
        command.getCommand(),
        se
      );
      authenticate();
  ...
  public synchronized void sendCommands(List<MPDCommand> commandList) {
    try {
      ...
      LOGGER.warn(
        "Connection exception while sending commands, will retry",
        se
      );
      authenticate();
      mpdSocket.sendCommands(commandList);
  ...
  private void checkSocket() {
    if (mpd == null) {
      throw new MPDConnectionException(
        "Socket could not be established.  Was mpd set?"
      );
    }
