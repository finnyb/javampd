...
public class MPDSongDatabase implements SongDatabase {
  ...
  @Override
  public Collection<MPDSong> findAlbumByArtist(
    MPDArtist artist,
    MPDAlbum album
  ) {
  ...
  public Collection<MPDSong> findAlbumByArtist(
    ...
    List<MPDSong> songList = new ArrayList<>(
      songSearcher.find(SongSearcher.ScopeType.ALBUM, albumName)
    );
  ...
  public Collection<MPDSong> findAlbumByGenre(MPDGenre genre, MPDAlbum album) {
    List<MPDSong> songList = new ArrayList<>(
      songSearcher.find(SongSearcher.ScopeType.ALBUM, album.getName())
    );
    ...
    return songList
      ...
      .filter(
        song ->
          song.getGenre() != null && song.getGenre().equals(genre.getName())
      )
      .collect(Collectors.toList());
  }
  ...
  @Override
  public Collection<MPDSong> searchTitle(
    String title,
    int startYear,
    int endYear
  ) {
    ...
    for (MPDSong song : songSearcher.search(
      SongSearcher.ScopeType.TITLE,
      title
    )) {
      int year;
  ...
  public Collection<MPDSong> searchFileName(String fileName) {
    return songSearcher.search(
      SongSearcher.ScopeType.FILENAME,
      removeSlashes(fileName)
    );
  }
  ...
  public MPDSong findSong(String name, String album, String artist) {
    List<MPDSong> songs = new ArrayList<>(
      songSearcher.find(SongSearcher.ScopeType.ALBUM, album)
    );
    ...
    LOGGER.info(
      "Song not found title --> {}, artist --> {}, album --> {}",
      name,
      artist,
      album
    );
