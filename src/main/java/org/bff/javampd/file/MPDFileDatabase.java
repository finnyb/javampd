...
public class MPDFileDatabase implements FileDatabase {
  ...
  private Collection<MPDFile> listDirectoryInfo(String directory) {
    ...
    List<String> commandResponse = commandExecutor.sendCommand(
      databaseProperties.getListInfo(),
      directory
    );
    ...
    while (iterator.hasNext()) {
      if (
        line == null ||
        (!line.startsWith(PREFIX_FILE) && !line.startsWith(PREFIX_DIRECTORY))
      ) {
        line = iterator.next();
      }
