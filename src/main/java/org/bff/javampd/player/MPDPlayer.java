...
public class MPDPlayer implements Player {
  ...
  public MPDSong getCurrentSong() {
    List<MPDSong> songList = songConverter.convertResponseToSong(
  ...
  @Override
  public synchronized void removePlayerChangedListener(
    PlayerChangeListener pcl
  ) {
   ...
   */
  protected synchronized void firePlayerChangeEvent(
    PlayerChangeEvent.Event event
  ) {
  ...
  @Override
  public synchronized void removeVolumeChangedListener(
  ...
  public Status getStatus() {
    ...
    if (currentStatus.equalsIgnoreCase(Status.STATUS_PLAYING.getPrefix())) {
      ...
      currentStatus.equalsIgnoreCase(Status.STATUS_PAUSED.getPrefix())
    ) {
