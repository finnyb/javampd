...
public interface Player {
  ...
  /**
   * Adds a {@link PlayerChangeListener} to this object to receive {@link PlayerChangeEvent}s.
   *
  ...
  /**
   * Adds a {@link VolumeChangeListener} to this object to receive {@link VolumeChangeEvent}s.
   *
  ...
  /**
   * Seeks to the desired location in the current song. If the location is larger than the length of
   * the song or is less than 0 then the parameter is ignored.
   *
  ...
  /**
   * Seeks to the desired location in the specified song. If the location is larger than the length
   * of the song or is less than 0 then the parameter is ignored.
   *
  ...
  /**
   * Sets the volume of the player. The volume is between 0 and 100, any volume less that 0 results
   * in a volume of 0 while any volume greater than 100 results in a volume of 100.
   *
  ...
  /**
   * Returns the {@link org.bff.javampd.audioinfo.MPDAudioInfo} about the current status of the
   * player. If the status is unknown {@code null} will be returned. Any individual parameter that
   * is not known will be a -1
   *
  ...
  /**
   * The threshold at which songs will be overlapped. Like crossfading but doesn’t fade the track
   * volume, just overlaps.
   *
   ...
   * track volume, just overlaps. The songs need to have MixRamp tags added by an external tool. 0dB
   * is the normalized maximum volume so use negative values
   *
  ...
  /**
   * Additional time subtracted from the overlap calculated by mixrampdb. A value of <code>0
   * </code> disables MixRamp overlapping and falls back to crossfading.
   *
