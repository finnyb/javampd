...
public class MPDOutputMonitor implements OutputMonitor {
  ...
  public void checkStatus() {
    ...
    if (outputs.size() > outputMap.size()) {
      ...
      fireOutputChangeEvent(
          new OutputChangeEvent(this, OutputChangeEvent.OUTPUT_EVENT.OUTPUT_DELETED));
      loadOutputs(outputs);
    } else {
  ...
  private void compareOutputs(List<MPDOutput> outputs) {
    for (MPDOutput output : outputs) {
      ...
      if (mpdOutput.isEnabled() != output.isEnabled()) {
        fireOutputChangeEvent(
            new OutputChangeEvent(output, OutputChangeEvent.OUTPUT_EVENT.OUTPUT_CHANGED));
        loadOutputs(outputs);
        return;
      }
    }
  ...
  /**
   * Sends the appropriate {@link OutputChangeEvent} to all registered {@link
   * org.bff.javampd.output.OutputChangeListener}s.
   *
