...
public class MPDPlayerMonitor
  ...
  public void checkStatus() {
    ...
    if (state.startsWith(StandAloneMonitor.PlayerResponse.PLAY.getPrefix())) {
      ...
      state.startsWith(StandAloneMonitor.PlayerResponse.PAUSE.getPrefix())
    ) {
      newStatus = STATUS_PAUSED;
    } else if (
      state.startsWith(StandAloneMonitor.PlayerResponse.STOP.getPrefix())
    ) {
  ...
  protected synchronized void firePlayerChangeEvent(
    PlayerBasicChangeEvent.Status status
  ) {
