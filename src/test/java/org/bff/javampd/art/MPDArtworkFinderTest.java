...
public class MPDArtworkFinderTest {
  ...
  public void findArtist() throws IOException {
    ...
    String prefix =
      decode(
        new File(
          this.getClass()
            .getResource("/images/artist/" + artistImages[0])
            .getFile()
        )
        .getParent()
      ) +
    ...
    btyeMap.put(
      artistImages[0],
      Files.readAllBytes(new File(prefix + artistImages[0]).toPath())
    );
    btyeMap.put(
      artistImages[1],
      Files.readAllBytes(new File(prefix + artistImages[1]).toPath())
    );
    btyeMap.put(
      albumImages[0],
      Files.readAllBytes(
        new File(prefix + albumSuffix + albumImages[0]).toPath()
      )
    );
    btyeMap.put(
      albumImages[1],
      Files.readAllBytes(
        new File(prefix + albumSuffix + albumImages[1]).toPath()
      )
    );
    ...
    Arrays
      ...
      .forEach(
        image -> {
          ...
          assertTrue(
            Arrays.equals(
              btyeMap.get(foundArtwork.getName()),
              foundArtwork.getBytes()
            )
          );
        }
      );
  ...
  public void findArtistBadPath() throws UnsupportedEncodingException {
    ...
    String path1 = decode(
      new File(
        this.getClass().getResource("/images/" + artistImages[0]).getFile()
      )
      .getParent()
    );
    List<MPDSong> songs = new ArrayList<>();
  ...
  public void findAlbum() throws UnsupportedEncodingException {
    ...
    String path1 = decode(
      new File(
        this.getClass()
          .getResource("/images/artist/album/" + albumImages[0])
          .getFile()
      )
      .getParent()
    );
  ...
  public void findPath() throws UnsupportedEncodingException {
    ...
    String testImage = decode(
      new File(
        this.getClass().getResource("/images/artist/" + images[0]).getFile()
      )
      .getParent()
    );
  ...
  public void findPathDirectoryIOException() {
    ...
    assertThrows(
      MPDException.class,
      () -> artworkFinder.find(finalTestFile.getParent())
    );
  }
